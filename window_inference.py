...
def get_clip_pred(
    ...
    for b, i in enumerate(slice_positions):
        windows.append(process_window(x[i : i + win_len], sr, audio_settings))
        ...
        if (not (b + 1) % batch_size) or (b + 1) == len(slice_positions):
            ...
            conf, preds = conf.cpu().numpy().reshape(
                -1, 1
    ...
    if len(result):
        ...
        if mode == "max":
            ...
            if label_map is not None:
                pred = list(
                    map(lambda a: [label_map[str(int(a[0]))], a[1], a[2], a[3]], result)
                )
            else:
                pred = result.tolist()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--device", type=str, default="auto", help="One of auto, cpu, or cuda."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Window length. E.g. for wlen = 1, will make inference on 1s windows from the clip.",
    )
    parser.add_argument(
        "--stride",
        type=float,
        default=0.2,
        help="By how much the sliding window will be shifted.",
    )
    parser.add_argument(
        "--thresh",
        type=float,
        default=0.85,
        help="Confidence threshold above which preds will be counted.",
    )
    parser.add_argument(
        "--mode",
        type=str,
        default="multi",
        help="""Prediction logic. One of: max, n_voting, multi.
