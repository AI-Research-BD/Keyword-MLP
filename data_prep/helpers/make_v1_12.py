...
def make_unknown(data_list, unknown_perc, silence_perc, label_map, root):
    ...
    unknown_frac = num_unknown / len(unknown_list) # fraction of unknown list which will go into data_list

    # use train_test_split to sample unknowns with stratification
    unknown_list, _, _, _ = train_test_split(unknown_list, unknown_class_list, train_size=unknown_frac,
                            random_state=0, shuffle=True, stratify=unknown_class_list)
...
def create_silence(root, train_list, val_list, total_train_size, total_val_size):
    ...
    train_val_silence, to_be_removed_silence, train_val_silence_source, _ = train_test_split(all_silence, silence_source, train_size=silence_frac,
                            random_state=0, shuffle=True, stratify=silence_source)
    silence_train_frac = num_silence_train / len(train_val_silence)
    train_silence, val_silence, _, _ = train_test_split(train_val_silence, train_val_silence_source, train_size=silence_train_frac,
                            random_state=0, shuffle=True, stratify=train_val_silence_source)
...
def main(args):
    ...
    train_list, val_list = create_silence(args.root, train_list, val_list, total_train_size, total_val_size)
