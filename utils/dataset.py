...
def get_train_val_test_split(root: str, val_file: str, test_file: str):
    ...
    with open(val_file, "r") as f:
        val_files_set = set(
            map(lambda a: os.path.join(root, a), f.read().rstrip("\n").split("\n"))
        )
    ...
    assert (
        len(val_files_set.intersection(test_files_set)) == 0
    ), "Sanity check: No files should be common between val and test."
...
class GoogleSpeechDataset(Dataset):
    ...
    def __init__(
        ...
        if root != "":
            data_list = list(
                map(lambda a: os.path.join(root, *a.split("/")[-2:]), data_list)
            )

        if cache:
            print("Caching dataset into memory.")
            self.data_list = init_cache(
                data_list, audio_settings["sr"], cache, audio_settings
            )
        else:
    ...
    def transform(self, x):
        ...
        if self.cache < 2:
            ...
            x = librosa.feature.mfcc(
                S=librosa.power_to_db(x), n_mfcc=self.audio_settings["n_mels"]
            )
            x = x.T  # (F, T) -> (T, F)
...
def cache_item_loader(
    path: str, sr: int, cache_level: int, audio_settings: dict
) -> np.ndarray:
...
def init_cache(
    ...
    loader_fn = functools.partial(
        cache_item_loader, sr=sr, cache_level=cache_level, audio_settings=audio_settings
    )
